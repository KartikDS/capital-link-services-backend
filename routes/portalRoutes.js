...
const { optionalAuthToken, authenticateToken } = require('../middleware/auth');
...
router.post('/documents', optionalAuthToken, upload.single('file'), portalController.uploadPortalDocument);

// Passport Photos
router.get('/passport-photos', optionalAuthToken, portalController.getPassportPhotos);
router.post('/passport-photos', optionalAuthToken, upload.single('photo'), portalController.uploadPassportPhoto);
