...
export const orderPaths = {
  ...
  '/api/orders/{reference}/comments': {
    get: operation('/api/orders/{reference}/comments', {
      ...
      description:
        'Both note tables, merged into one thread, newest first — see the `Comment` schema for which is which and why the ids differ. A consultant’s messages are in `tbl_order_destination_notes`, so a read of `tbl_order_notes` alone returned an order’s form summary and none of its correspondence.' + NL + NL + '`is_admin` separates two lanes, and **on the destination table it means the opposite of what CLS’s admin labels suggest:** the box labelled “Client comment” writes 0 and is the message *to* the client — `ViewOrderController` emails it to them on save — while “Admin comment” writes 1 and is CLS’s own working note. Only the first is returned: the internal lane is CLS’s confidential working record on the order and no client-facing route exposes it, the attachment download included. It was returned for one day, 2026-08-26 to 2026-08-27, at CLS’s request and then against it. On `tbl_order_notes` the same flag still filters: its 1s are the chargeable “Notary / DFAT / $85” fee lines, reported through the payments and invoice routes instead.',
      auth: 'bearer',
      responses: {
  ...
  '/api/orders/{reference}/comments/{id}/attachment': {
    get: operation('/api/orders/{reference}/comments/{id}/attachment', {
      ...
      description:
        'Takes the prefixed id the thread returns — `dn-1841`. Only a note on one of *this* order’s destination rows is served, so a note id on its own reaches nothing: the ids are small integers on a MyISAM table with no order column, and without that check the route would be a way of counting through other clients’ attachments. The note must also be on the client-facing lane: a file attached to a CLS working note (`is_admin = 1`) is as confidential as the note itself and is refused with the same wording as one that does not exist, so the route cannot be used to reach the internal lane by guessing ids.' + NL + NL + '`attachment` holds a bare filename — the old application moved the upload to `web/dev/destination_notes_file/` and stored only the basename — so the file is looked for at that path relative to the upload roots and then by the bare name, covering a `LEGACY_UPLOAD_DIR` mounted at the old web root or at that one folder. Unset, or a miss in both, answers 404 saying the record exists and the file does not.',
      auth: 'bearer',
      responses: {
        200: {
